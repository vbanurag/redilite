...
It also includes a built-in **RESP Protocol TCP Server** (so standard tools like `redis-cli`, `redis-py`, `go-redis`, `ioredis`, or `jedis` can connect seamlessly), an **Interactive CLI REPL**, and a full [Command Manual (COMMANDS.md)](COMMANDS.md).
